...
#transformed values. 
library(FNN)
filenames <- list.files(path = "../External/Oxford/Exported_csv/ILC_NK_all", 
                        full.names = TRUE)
...
big_all_file <- do.call("rbind", lapply(filenames, function(x){
  ...
  id <- gsub(pattern="../External/Oxford/Exported_csv/ILC_NK_all/ILCNK_all_|_.+\\.csv", "\\1", x)
  ret$id <- id
  tissue <- gsub(pattern="../External/Oxford/Exported_csv/ILC_NK_all/ILCNK_all_.+_|\\.csv", "\\1", x)
  ret$tissue <- tissue 
  ret 
}))
...
dir.create("../External/Oxford/Resulting_data/ILC_NK_all")
saveRDS(big_all_file, "../External/Oxford/Resulting_data/ILC_NK_all/ILC_NK_all_full_file.rds")
