...
for(i in cellPops){
  ...
  if(identical(row.names(locMetaDatPat), row.names(locDatPat))){
    ...
    if(sum(countTabFilt) > 50){
      ...
      #We will continue to call this "lymphVec" although it is not - it is cell
      #type specific. 
      if(i == "ILC"){
        lymphVec <- unlist(lapply(row.names(countTabFilt), function(x){
          length(which(sthlmProtDat$sample == gsub(".+_age_.._|", "", x) &
                         sthlmProtDat$cellType == "NK"))
        }))
      } else if(i == "TCRgd"){
        lymphVec <- unlist(lapply(row.names(countTabFilt), function(x){
          length(which(sthlmProtDat$sample == gsub(".+_age_.._|", "", x) &
                         sthlmProtDat$cellType == "gdT"))
        }))
      } else {
        lymphVec <- unlist(lapply(row.names(countTabFilt), function(x){
          length(which(sthlmProtDat$sample == gsub(".+_age_.._|", "", x) &
                         sthlmProtDat$cellType == i))
        }))
      }
...
dir.create("Results_per_celltype/Data/Oxford_and_Stockholm/Harmonisation", recursive = TRUE)
write.csv(pValDf, "Results_per_celltype/Data/Oxford_and_Stockholm/Harmonisation/pValTable_Stockholm.csv")

#We now exclude the ones that are not significant. 
pValDfSig <- pValDf[which(pValDf[,1] < 0.05),]
round(pValDfSig, 4)

#This is rather interesting: seemingly, this well corresponds to the finding in the other 
#analysis
#CD4T_EOMGlow_3 CD8T_LOMGlow_2   NK_EOMGlow_1 
#        0.0306         0.0119         0.0018 

write.csv(pValDfSig, "Results_per_celltype/Data/Oxford_and_Stockholm/Harmonisation/Sign_in_Stockholm.csv")
...
write.csv(freqTabBig, "Results_per_celltype/Data/Oxford_and_Stockholm/Harmonisation/freqTable_Stockholm.csv")
