...
dir.create("Results_per_celltype/Data/Oxford/Analysis_post_smooth", recursive = TRUE)
dir.create("Diagnostics_per_celltype/Oxford/BIC_model_plots", recursive = TRUE)
...
for(x in popVec){
  print(x)
  locFile <- readRDS(paste0("../External/Oxford/Resulting_data_analysis_per_celltype/", x, "/", x, "_full_file_post_Euclid.rds"))
  locRes <- bplapply(hitPops, function(y){
    if(length(which(locFile$SmoothGroup == y)) > 1){
      ...
      pdf(paste0("Diagnostics_per_celltype/Oxford/BIC_model_plots/", x, "_", y, "_model.pdf"))
      plot(res)
      dev.off()
...
saveRDS(subclusterability, "Results_per_celltype/Data/Oxford/Analysis_post_smooth/Subclusterability.rds")
#subclusterability <- readRDS("Results_per_celltype/Data/Oxford/Analysis_post_smooth/Subclusterability.rds")
...
for(i in popVec){
  print(i)
  datDir <- paste0("../External/Oxford/Resulting_data_analysis_per_celltype/", i, "/")
  ...
  #Here, we keep to the name lymphVec for convenience, but we change it, 
  #as this analysis focuses on per-cell type analyses
  #lymphVec <- unlist(lapply(row.names(countTab), function(x){
  #  locFile$lymphCount[which(locLongNameVec == x)[1]]
  #}))
  
  lymphVec <- unlist(lapply(row.names(countTab), function(x){
    length(which(locLongNameVec == x))
  }))
...
write.csv(pValDf, "Results_per_celltype/Data/Oxford/Analysis_post_smooth/pValTable.csv")
...
write.csv(freqTabBig, "Results_per_celltype/Data/Oxford/Analysis_post_smooth/freqTableAllPops.csv")
