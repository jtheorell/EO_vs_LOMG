...
for(a in popVec){
  print(a)
  dataDirIn <- paste0("../External/Oxford/Resulting_data/", a, "/")
  big_all_file <- readRDS(paste0(dataDirIn, a, "_full_file.rds"))
  dataDirOut <- paste0("../External/Oxford/Resulting_data_analysis_per_celltype/", a, "/")
  ...
  #Here, we turn the unclear into LOMGs, but their rows will never be used
  #as neighbors anyway, so this has no consequence apart from creating a less
  #messy groupDummy object (we would otherwise have one column that would 
  #be empty for all cells, as the unclear will always be 0 among the neighbors)
  ...
  #Ans this rather complex analysis is primarily done with the lmphocyte
  #population as reference, we will not stray far from this original concept here. 
  
  #THis is first of all meaningless, but we will keep it anyway. 
  big_all_file$pop_perc <- 1
  
  #Now we will introduce a new thing, namely the size of the individual pop. 
  big_all_file$popCount <- NA
  for(i in runVec){
    locRowVec <- which(big_all_file$idTime == i)
    big_all_file$popCount[locRowVec] <- length(locRowVec)
  }
  ...
  eomgDonMinSize <- min(big_all_file$popCount[eomgRows])
  ctrlDonMinSize <- min(big_all_file$popCount[ctrlRows])
  lomgDonMinSize <- min(big_all_file$popCount[lomgRows])
  ...
  indivSampSizes <- unlist(lapply(sampSizeList, function(y){
    locList <- lapply(y[[1]], function(z){
      ...
      locPopSize <- big_all_file$popCount[locRows][1]
  ...
  modelClusts <- readRDS(paste0(dataDirIn, "modelClusts.rds"))
  
  dir.create(paste0(dataDirOut, "NeighSmoothModels"))
  for(i in 1:100){
    ...
    saveRDS(neighSmoothRes, 
            paste0(dataDirOut, "NeighSmoothModels/neighSmoothRes_", i, ".rds"))
    if(i > 2){
      smoothFileList <- list.files(paste0(dataDirOut, "NeighSmoothModels"), 
                                   full.names = TRUE)
  ...
  saveRDS(big_all_file, paste0(dataDirOut, a, "_full_file_post_Euclid.rds"))
}