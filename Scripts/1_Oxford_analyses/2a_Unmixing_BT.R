...
refSet <- read.flowSet(path = "../External/Oxford/FCS/Raw/B_T_beads",
...
write.csv(rawMat, "Data/Oxford/Unmixing_matrices/Raw_mat_BT.csv")
#After export, the order of the rows and the naming is changed a bit. 

usedMat <- read.csv("Data/Oxford/Unmixing_matrices/Used_mat_BT.csv", row.names = 1,
                  check.names = FALSE)

#Now, we will run through this file-by file, as each file is so big. 
dir.create("../External/Oxford/FCS/Unmixed/BT", recursive = TRUE)
fileList <- list.files("../External/Oxford/FCS/Raw/B_T_panel", full.names = TRUE)
for(i in fileList){
  print(i)
  ...
  write.../External/Oxford/FCS(unmixFF, paste0("../External/Oxford/FCS/Unmixed/BT/Unmixed_", 
                            gsub("../External/Oxford/FCS/Raw/B_T_panel/", "", i)))
}
