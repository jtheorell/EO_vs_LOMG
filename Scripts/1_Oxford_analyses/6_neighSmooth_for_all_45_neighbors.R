...
library(ClusterR)
library(DepecheR)
dir.create("../External/Oxford/Resulting_data_45_neighbors/")
for(a in popVec){
  print(a)
  dataDir <- paste0("../External/Oxford/Resulting_data_45_neighbors/", a, "/")
  big_all_file_with_pJor <- readRDS(paste0("../External/Oxford/Resulting_data/", a, "/", a,  "_full_file.rds"))
  ...
  modelClusts <- readRDS(paste0("../External/Oxford/Resulting_data/", a, "/modelClusts.rds"))
  ...
  #gated out at the dump/dead gate stage), it will be excluded here, before going
  #in to the real analyses. 
  modelClusts$clusters <- modelClusts$clusters[-which(big_all_file_with_pJor$id == "PJor")]
  
  dir.create(paste0(dataDir, "NeighSmoothModels"), recursive = TRUE)
  for(i in 1:100){
  ...
  smoothFileList <- list.files(paste0(dataDir, "NeighSmoothModels"), full.names = TRUE)
  ...
  saveRDS(big_all_file, paste0(dataDir, a, "_full_file_post_Euclid_45_neighbors.rds"))
}