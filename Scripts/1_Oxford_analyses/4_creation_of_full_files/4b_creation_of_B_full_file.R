...
#transformed values. 
library(FNN)
filenames <- list.files(path = "../External/Oxford/Exported_csv/B", 
                        full.names = TRUE)

big_all_file <- do.call("rbind", lapply(filenames, function(x){
  ret <- read.csv(x)
  id <- gsub(pattern="../External/Oxford/Exported_csv/B/B_|_.+\\.csv", "\\1", x)
  ret$id <- id
  tissue <- gsub(pattern="../External/Oxford/Exported_csv/B/B_.+_|\\.csv", "\\1", x)
  ret$tissue <- tissue 
  ret 
}))
...
# 800619 1059762  252483  163651       0 
...
dir.create("../External/Oxford/Resulting_data/B")
saveRDS(big_all_file, "../External/Oxford/Resulting_data/B/B_full_file.rds")
