...
library(FNN)
filenames <- list.files(path = "../External/Oxford/Exported_csv/CD8T", 
                        full.names = TRUE)

big_all_file <- do.call("rbind", lapply(filenames, function(x){
  ret <- read.csv(x)
  id <- gsub(pattern="../External/Oxford/Exported_csv/CD8T/CD8T_|_.+\\.csv", "\\1", x)
  ret$id <- id
  tissue <- gsub(pattern="../External/Oxford/Exported_csv/CD8T/CD8T_.+_|\\.csv", "\\1", x)
  ret$tissue <- tissue 
  ret 
}))
...
#1807171 1545664  549730  193630       0 
...
dir.create("../External/Oxford/Resulting_data/CD8T")
saveRDS(big_all_file, "../External/Oxford/Resulting_data/CD8T/CD8T_full_file.rds")
