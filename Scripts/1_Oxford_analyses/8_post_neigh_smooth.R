...
for(x in popVec){
  print(x)
  locFile <- readRDS(paste0("../External/Oxford/Resulting_data/", x, "/", x, "_full_file_post_Euclid_consensus.rds"))
...
penaltyList <- list("B" = standardPenalties,
                    "CD4T" = list("EOMGhigh" = 2^seq(4, 8, by = 0.5),
                                  "EOMGlow" = 2^seq(7, 9, by = 0.5),
                                  "LOMGhigh" = 2^seq(6, 9, by = 0.5),
                                  "LOMGlow" = 2^seq(6, 9, by = 0.5)),
                    "CD8T" = list("EOMGhigh" = 2^seq(5, 9, by = 0.5),
                                  "EOMGlow" = 2^seq(5, 9, by = 0.5),
                                  "LOMGhigh" = 2^seq(5, 9, by = 0.5),
                                  "LOMGlow" = 2^seq(6, 9, by = 0.5)),
                    "TCRgd" = list("EOMGhigh" = 2^seq(7, 9, by = 0.5),
                                   "EOMGlow" = 2^seq(5, 9, by = 0.5),
                                   "LOMGhigh" = 2^seq(5, 9, by = 0.5),
                                   "LOMGlow" = 2^seq(5, 9, by = 0.5)),
...
#Now, what percentage of cells is present in these populations?
#freqTabBig <- read.csv("Results/Data/Oxford/Analysis_post_smooth/freqTableAllPops.csv", row.names = 1)
summary(unlist(lapply(popVec, function(x){
  locPop <- freqTabBig[,grep(x, colnames(freqTabBig))]
  #Now, what is the combined percentage of all the populations, per donor?
  locPercent <- rowSums(locPop)
  median(locPercent)
})))
